...
static string
join_source(const vector<bool> &s, unordered_map <size_t, string> &fidmap,
            string sep) {
  string js;
...
static Endpoint
endpoint_from_interval(const GenomicRegion &r, const bool isfirst,
  ...
  if (isfirst)
    ...
    start = r.get_end();
  Endpoint new_endpoint = Endpoint(r.get_chrom(), start, isfirst);
  new_endpoint.source = source;
  return new_endpoint;
}
...
load_units(const size_t &num_files, const string &interval_file,
    ...
    while (getline(in, buffer)) {
      ...
      endpoints.push_back(endpoint_from_interval(interval, true, sources));
      endpoints.push_back(endpoint_from_interval(interval, false, sources));
    }
}
...
rm_junk(const float &min_del_freq, const size_t &min_size,
  ...
  for (size_t i = 0; i < smallunits.size(); ++i) {
    ...
    //             static_cast<float> (smallunits[i].icover);
...
static void
rm_zero_density(vector<Subunit> &subunits) {
  size_t j = 0;
  for (size_t i = 0; i < subunits.size(); ++i) {
    if (!subunits[i].zero_density()) subunits[j++] = subunits[i];
  }
  subunits.erase(subunits.begin() + j, subunits.end());
}
...
generate_merged_subunit(Subunit &x, Subunit &y, char core, float &mscore,
  ...
  float sep_score = x.score_sum() + y.score_sum();
  if (sep_score == 0) 
    mscore = 0;
  else 
    mscore = static_cast<float>(z.score_sum()) / static_cast<float>(sep_score);
  return z;
}
...
merge_two(const float &degcutoff, const vector<idxed_val> &scores,
  ...
  if (mscore >= degcutoff) {
    //std::cout << "score: " << mscore << endl;
    has_merged = true;
    if (mscore_left > mscore_right) {
      island[i-1] = lhs;
      island.erase(island.begin() + i);
      if (island[i-1].empty()) island.erase(island.begin() + i-1);
    } else {
      island[i] = rhs;
      island.erase(island.begin() + i+1);
      if (island[i].empty()) island.erase(island.begin() + i);
    }
  }
  return has_merged;
}
...
subunits_to_file(const string &outfile, const vector<Subunit> &subunits,
  ...
  for (size_t i = 0; i < subunits.size(); ++i) {
    float freq = static_cast<float> (subunits[i].count) /
                 static_cast<float> (subunits[i].icover);
     out << subunits[i].chr << '\t'
          << subunits[i].start << '\t' << subunits[i].end << '\t'
          << join_source(subunits[i].source, fidmap, ",") << '\t'
          << freq << '\t'
...
main(int argc, const char **argv) {
  ...
  try {
    ...
    rm_zero_density(subunits);
    subunits_to_file(outfile, subunits, fidmap);
  }
