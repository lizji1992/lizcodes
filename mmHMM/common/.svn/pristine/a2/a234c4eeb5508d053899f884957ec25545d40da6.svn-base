...
ThreeStateVDHMM::ThreeStateVDHMM(
...
void
ThreeStateVDHMM::set_parameters(
...
void
ThreeStateVDHMM::get_parameters(
...
//////////////////////////////////////////////
void
ThreeStateVDHMM::update_observation_likelihood()
{
...
double
ThreeStateVDHMM::gain_segment_log_likelihood(
...
double
ThreeStateVDHMM::same_segment_log_likelihood(
    const size_t start, const size_t end)
{
    return
        (start == 0)
...
double
ThreeStateVDHMM::loss_segment_log_likelihood(
    const size_t start, const size_t end)
{
    return
        (start == 0)
...
double
ThreeStateVDHMM::forward_algorithm(const size_t start, const size_t end) 
{
/////
...
/////
    ...
    forward[start].same =
        ...
        + same_duration.log_likelihood(1);
    forward[start].loss =
    ...
    for (size_t i = start + 1; i < end; ++i)
    {
        ...
        for (size_t l = 1; l < max_len; ++l)
        {
            ...
            // background segment
            const double same_seg_llh = (beginning == start) ?      
                lp_start.same
                + same_segment_log_likelihood(beginning, ending)
                + same_duration.log_likelihood(l)
                :
                log_sum_log(forward[beginning - 1].gain + log(trans[GAIN][SAME]),
                            forward[beginning - 1].loss + log(trans[LOSS][SAME]))
                + same_segment_log_likelihood(beginning, ending)
                + same_duration.log_likelihood(l);

            forward[i].same = log_sum_log(forward[i].same, same_seg_llh);
        }
        ...
        assert(finite(forward[i].gain));
        assert(finite(forward[i].same));
        assert(finite(forward[i].loss));
    }
    return log_sum_log(forward[end - 1].gain + lp_end.gain,
                       log_sum_log(forward[end - 1].same + lp_end.same,
...
double
ThreeStateVDHMM::backward_algorithm(const size_t start, const size_t end)
{
// /////
    ...
    for (size_t i = end - 2; i >= start; --i)
    {
        ...
        for (size_t l = 1; l < max_len; ++l)
        {
            ...
            const double same_seg_llh = 
                ...
                + same_segment_log_likelihood(beginning, ending)
                + same_duration.log_likelihood(l)
                + backward[ending - 1].same;
            backward[i].gain = log_sum_log(backward[i].gain, same_seg_llh);
        }
        ...
        for (size_t l = 1; l < max_len; ++l)
        {
            ...
            const double same_seg_llh = 
                ...
                + same_duration.log_likelihood(l)
                + backward[ending - 1].same;
            backward[i].loss = log_sum_log(backward[i].loss, same_seg_llh);
        }

        assert(finite(backward[i].gain));
        assert(finite(backward[i].same));
        assert(finite(backward[i].loss));
...
// Expectation
void
ThreeStateVDHMM::estimate_state_posterior(const size_t start, const size_t end)  
{
// /////
...
// /////
    ...
    for (size_t s = start; s < end; ++s)
    {
        ...
        for (size_t e =  min(s + MAX_LEN, end); e > s; --e)
        {
            const double evidence = (s == start) ?
                lp_start.same
                + same_duration.log_likelihood(e - s)
                + same_segment_log_likelihood(s, e)
                + backward[e - 1].same
                :
                log_sum_log(forward[s - 1].gain + log(trans[GAIN][SAME]),
                            forward[s - 1].loss + log(trans[LOSS][SAME]))
                + same_duration.log_likelihood(e - s)
                + same_segment_log_likelihood(s, e)
                + backward[e - 1].same;
            
            accu_evidence = log_sum_log(accu_evidence, evidence);
            same_evidence[e - 1 - start] =
                log_sum_log(same_evidence[e - 1 - start], accu_evidence);
        }
    }
    ...
    for (size_t i = start; i < end; ++i)
    {
        const double denom =
            log_sum_log(gain_evidence[i - start],
                        log_sum_log(same_evidence[i - start],
...
void 
ThreeStateVDHMM::estimate_parameters()
{
// /////
...
//    static const bool BG_STATE = false;
    ...
    for (size_t idx = 0; idx < reset_points.size() - 1; ++idx)
    {
        ...
        STATE_LABELS prev_state =
            max3(gain_posteriors[start], same_posteriors[start],
                 loss_posteriors[start]);
        size_t len = 1;
        for (size_t i = start + 1; i < end; ++i)
        {
            const STATE_LABELS state = 
                max3(gain_posteriors[i], same_posteriors[i], loss_posteriors[i]);
            if (state == prev_state)
                ++len;
            else
            {
    ...
    trans[SAME][GAIN] = gain_lengths.size()
        / (gain_lengths.size() + loss_lengths.size());

    trans[SAME][LOSS] = loss_lengths.size()
        / (gain_lengths.size() + loss_lengths.size());
...
double
ThreeStateVDHMM::single_iteration()
{
// /////
...
double
ThreeStateVDHMM::BaumWelchTraining()  
{
// /////
...
void
ThreeStateVDHMM::get_posterior_scores(
    ...
    std::vector<STATE_LABELS> & classes)
{
    ...
    for (size_t i = 0; i < observations.size(); ++i)
        classes[i] = max3(gain_posteriors[i],
                          same_posteriors[i],
                          loss_posteriors[i]);
