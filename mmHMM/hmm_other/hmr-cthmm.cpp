...
main(int argc, const char **argv) 
{
    ...
    else
    {
      ...
      hmm.set_parameters(fg_emission, bg_emission,
          fg_duration, bg_duration);
      if (VERBOSE)
        cerr << "MODEL TRAININING: OVER" << endl << endl;
      if (max_iterations > 0) 
    ...
    vector<double> scores;
    if (VERBOSE)
      cerr << "[POSTERIOR DECODING]" << endl << endl;
    hmm.PosteriorDecoding();
    hmm.get_posterior_scores(scores, classes);
    // write posterior probabilities if needed
    if (!scores_file.empty()) {
      std::ofstream ofs;
      ofs.open(scores_file.c_str());
      std::ostream outs(scores_file.empty() ? std::cout.rdbuf() : ofs.rdbuf());
      for (size_t i = 0; i < cpgs.size(); ++i)
        outs << cpgs[i] << '\t' << scores[i] << '\n';
    }
    // get domains and domain scores
    if (VERBOSE)
      cerr << "[BUILD DOMAINS]" << endl << endl;
